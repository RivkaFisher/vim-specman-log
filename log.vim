" Vim syntax file for specman log
" Language:         specman log file
" Maintainer:      MTDL9 <https://github.com/MTDL9>; rivkafisher100@gmail.com 
" Latest Revision:  2022-03-10
...
"%%%%%%%%%%%%%%%% specman %%%%%%%%%%%%%%%%
syn match dut_error ".*Dut error.*"
syn match aserttion ".*E,ASRTST.*"
syn match error ".*ASRT`.*"
syn match error ".*ERROR.*"
syn match warn ".*WARN.*"
syn match info ".*INFO.*"
syn match debug ".*DEBUG.*"
syn match test_message ".*TEST_MESSAGE.*"
syn match matlab_excute ".*VENVHOME.*"
syn match end_test ".*Checking is complete.*"


hi aserttion guifg=Red
hi dut_error ctermfg=Red  guifg=Red 
hi test_message guifg =Yellow
hi error ctermfg=magenta guifg=magenta guibg=Black
hi warn ctermfg=Yellow guifg=Yellow
hi info ctermfg=Green  guifg=Green 
hi debug ctermfg=Gray  guifg=Gray 
hi matlab_excute guifg=Orange
hi end_test guibg=Yellow guifg=Blue
"hi dut_error ctermfg=Red ctermbg=Black guifg=Red guibg=Black
